...
void Adafruit_ILI9341::begin(uint32_t freq) {
  ...
  if (_rst < 0) {                 // If no hardware reset pin... rst = -1.  Pulled high with resistor.
    sendCommand(ILI9341_SWRESET); // Engage software reset
    sleep_ms(150);
  }

  uint8_t cmd, x, numArgs;
  const uint8_t *addr = initcmd;  // initcmd is a pointer to the beginning of the array.
  
  while ((cmd = *addr++) > 0) {
    x = *addr++;   // De-reference the pointer.
    numArgs = x & 0x7F;  // Mask the MSB.  Not sure why.
    sendCommand(cmd, addr, numArgs);
    addr += numArgs;  // Jump over the command arguments to the next command.
    if (x & 0x80)
      sleep_ms(150);
  }
